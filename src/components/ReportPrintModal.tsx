...
export const ReportPrintModal: React.FC<ReportPrintModalProps> = ({
  ...
  const clientName = report.clientName ? report.clientName[isAr ? 'ar' : 'en'] : (isAr ? 'ميسون إبراهيم سعيد قنيطة' : 'Maysoon Ibrahim Saeed Qunaita');
  const branchNumber = report.branchNumber || '57';
            ...
            <div className="flex items-center justify-between border-b-2 border-[#1d1d1f] pb-4 sm:pb-5 gap-2">
              <div className="flex items-center gap-2.5 sm:gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] sm:text-xs text-[#8B6F47] font-bold">
                    {isAr ? 'الفرع: مثوى ٥٧ • حي السحمان' : 'Branch: Mathwaa 57 • Al-Sahman District'}
                  </p>
                  <p className="text-[10px] sm:text-[11px] text-stone-600 font-semibold mt-0.5">
                    {isAr ? `عدد الوحدات: ${report.totalUnits || 3} وحدات` : `Number of Units: ${report.totalUnits || 3} units`}
                  </p>
                </div>
              </div>
              <div className="text-right text-xs flex-shrink-0">
                ...
                <p className="text-[9px] sm:text-[10px] text-stone-400 font-mono mt-1 hidden sm:block">
                  Ref: {report.payoutRef || 'TXN-20260731-MTH57'}
                </p>
              </div>
            </div>
                ...
                <table className="w-full text-xs border-collapse">
                  ...
                  <tbody>
                    ...
                    <tr className="border-b border-[#EDE5DC] bg-[#FAF7F2]">
                      ...
                      <td className="p-2.5 sm:p-3 font-extrabold text-[#8B6F47] text-left">{formatCurrency(report.avgMonthlyReturn || 6278.30)} SAR</td>
                    </tr>
                    <tr className="border-b border-[#EDE5DC]">
                      <td className="p-2.5 sm:p-3 font-bold">• {isAr ? 'المصاريف المباشرة' : 'Direct Expenses'}</td>
                      <td className="p-2.5 sm:p-3 font-extrabold text-rose-700 text-left">{formatCurrency(report.directExpenses || 727.76)} SAR</td>
                    </tr>
                    <tr className="border-b border-[#EDE5DC] bg-[#FAF7F2]">
                    ...
                    <tr className="border-b border-[#EDE5DC]">
                      ...
                      <td className="p-2.5 sm:p-3 font-extrabold text-stone-600 text-left">{formatCurrency(report.capitalExpenses ?? 0)} SAR</td>
                    </tr>
